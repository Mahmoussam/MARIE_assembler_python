...
class XMLHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent=None):
        ...
        labelFormat.setForeground(QColor('#ffff00')) #yellow
        self.highlightingRules.append((QRegExp("\\b\w+,"), labelFormat))
        #single line comment
        singleLineCommentFormat = QTextCharFormat()
        singleLineCommentFormat.setForeground(QColor("#a0a0a4")) #grey
        self.highlightingRules.append((QRegExp("/[^\n]*"), singleLineCommentFormat))
        ...
        directiveFormat.setForeground(QColor(255,0,0))
        directives=['HEX','DEC']
        dirsPatterns = []
        for d in directives:
            pat=''
            for c in d:
                pat+='[{}]'.format(c.upper()+c.lower())
            dirsPatterns.append('\\b'+pat+'\\b')
        
        self.highlightingRules += [(QRegExp(pattern), directiveFormat)
                for pattern in dirsPatterns]
        #keywords 
        op_s=['ADD','SUBT','ADDI','CLEAR','LOAD','STORE','INPUT','OUTPUT','JUMP','SKIPCOND','JNS','JUMPI','LOADI','STOREI','HALT']
        keywordPatterns = []
        for op in op_s:
            pat=''
            for c in op:
                pat+='[{}]'.format(c.upper()+c.lower())
            keywordPatterns.append('\\b'+pat+'\\b')
        keywordFormat = QTextCharFormat()
        keywordFormat.setForeground(QColor("#e35e00")) #orange
...
class QCodeEditor(QPlainTextEdit):
    ...
    class NumberBar(QWidget):
        ...
        def __init__(self, editor):
            ...
            self.font = QFont()
            self.numberBarColor = QColor(30, 30, 30)
    ...
    def __init__(self, DISPLAY_LINE_NUMBERS=True, HIGHLIGHT_CURRENT_LINE=True,
                 SyntaxHighlighter=None, Parent=None,*args):        
        '''
        Parameters
        ----------
        ...
        if HIGHLIGHT_CURRENT_LINE:
            ...
            self.currentLineColor = QColor(45,45,45)
